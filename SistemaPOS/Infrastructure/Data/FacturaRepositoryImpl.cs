...
namespace SistemaPOS.Infrastructure.Data
{
    public class FacturaRepositoryImpl : FacturaRepository
    {
        ...
        public async Task<string> ImprimirFactura(int facturaId)
        {
            ...
            Cliente cliente = facturaDatos.First().cliente;
            ...
            int contador = 1;
            foreach( var fd in facturaDatos)
            {
                clId = new PdfPCell(new Phrase(contador.ToString(), _standardFont));
                clNombre = new PdfPCell(new Phrase(fd.producto.Nombre, _standardFont));
                clValorUnitario = new PdfPCell(new Phrase(fd.producto.ValorUnitario.ToString(), _standardFont));
                clMedida = new PdfPCell(new Phrase(fd.producto.Medida.ToString(), _standardFont));
                clUnidadMedida = new PdfPCell(new Phrase(fd.producto.UnidadMedida, _standardFont));
                clPeso = new PdfPCell(new Phrase(fd.producto.Peso.ToString(), _standardFont));
                clCantidad = new PdfPCell(new Phrase(fd.pedido.Cantidad.ToString(), _standardFont));
                clValorTotal = new PdfPCell(new Phrase("$" + (fd.producto.ValorUnitario * fd.pedido.Cantidad), _standardFont));
